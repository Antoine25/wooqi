...
from wooqi.src import WOOQI_RERUNS_KEY, global_var
...
def item_loop_option_analyse(item, test_name, first_arg):
    ...
    """
    if item.get_closest_marker("loop") is not None:
        ...
        # Test defined multiple time in config file but not is the first iteration
            ...
            while global_var["config"].exist("{}_{}".format(test_name, cpt)):
                test_order = global_var["config"].file_config["{}_{}".format(test_name, cpt)]["test_order"]
...
def filter_order_tests(config, items):
        ...
        if session.startswith("test_") or session.startswith("action_"):
            ...
            if list_param:
                ...
                for params in list_param:
                    ...
                    test_name_with_param = "{}-{}]".format(test_name, concatenate_params).replace("[-", "[")
                    ...
                    if loop_iter:
                        if type(params) is tuple:
                            ...
                            iter_number = params
                        pytest_tests[test_name_with_param] = {
                            "session": session,
    ...
    for item in items:
        if item.name in pytest_tests:
            ...
            if "reruns" in file_config[session_name]:
                pytest_tests[item.name]["item"].stash[WOOQI_RERUNS_KEY] = 0
        ...
        elif item.get_closest_marker("parametrize") is not None:
            for pytest_test in pytest_tests:
                pytest_base_name = pytest_test.replace("]", "-") if "]" in pytest_test else "{}[".format(pytest_test)
    ...
    for test_name in pytest_tests:
        if "item" not in pytest_tests[test_name]:
            msg = "{} not found, please verify the test definition and your configuration file".format(test_name)
            print(msg)
            raise Exception(msg)
    ...
    ordered_list = sorted(
        pytest_tests,
        key=lambda k: (
            pytest_tests[k]["iter_number"],
            pytest_tests[k]["test_order"],
            k,
        ),
    )
    items_temp = []
...
def postfail_feature_management(item, skip):
    ...
    for session_item in item.session.items:
        if post_fail and (post_fail == session_item.name or post_fail.replace("]", "-") in session_item.name):
            break
